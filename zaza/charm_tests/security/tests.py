...
def make_test_function(application, file_details):
    def test(self):
        ...
        for unit in model.get_units(application):
            ...
            self.assertEqual(expected_owner,
                             owner,
                             "Owner is incorrect for {}: {}".format(unit, owner))
            self.assertEqual(expected_group,
                             group,
                             "Group is incorrect for {}: {}".format(unit, group))
            self.assertEqual(expected_mode,
                             mode,
                             "Mode is incorrect for {}: {}".format(unit, mode))
    return test
        ...
        for name, attributes in files.items():
            ...
            if name in deployed_applications:
              for file in attributes['files']:
                  test_func = make_test_function(name, file)
                  setattr(cls, 'test_{}_{}'.format(name, file['path']), test_func)
        return cls
...
FileOwnershipTest = add_tests()(FileOwnershipTest)
